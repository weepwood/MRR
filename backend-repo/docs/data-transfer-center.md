...
`mr_scan` 的两个数据交换索引由 `V0_9` 使用 `CREATE INDEX CONCURRENTLY` 创建，避免普通索引构建长时间阻塞写入。该迁移通过同名 `.sql.conf` 禁用 Flyway 事务。若数据库异常中断，必须先执行验收脚本确认索引处于 `indisvalid/indisready` 状态，再开放导入功能。
