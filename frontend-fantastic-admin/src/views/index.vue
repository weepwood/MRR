...
interface DashboardWidgetView extends DashboardWidgetDefinition, DashboardWidgetPreference {}
...
function handleWidgetCommand(command: string, widget: DashboardWidgetView) {
  if (command === 'edit') openWidgetEditor(widget.id)
  if (command === 'pin') toggleWidgetPinned(widget.id)
  if (command === 'hide') setWidgetVisible(widget.id, false)
  if (command.startsWith('size:')) handleSizeChange(widget.id, command.slice(5))
}
...
<template>
  <MrrPageShell width="fluid">
    ...
    <el-dialog v-model="managerVisible" title="管理小组件" width="760px" class="widget-manager-dialog">
