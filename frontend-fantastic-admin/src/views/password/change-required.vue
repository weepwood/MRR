...
const rules = {
  currentPassword: [{ required: true, message: '请输入管理员提供的临时密码', trigger: 'blur' }],
  newPassword: [
    ...
    <section class="password-card" aria-labelledby="password-change-title">
      ...
      <div class="card-heading">
        ...
        <p>
          ...
          当前使用管理员分配的临时密码。完成修改前无法访问病案、患者和系统管理功能。
        </p>
      </div>
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="临时密码仅用于完成本次身份确认；修改成功后当前登录状态会立即失效。"
      />

      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="password-form">
        <el-form-item label="当前临时密码" prop="currentPassword">
          <el-input
      ...
      <div class="password-rules">
        <strong>密码要求</strong>
        <span>长度 12–64 位；不能与临时密码相同；建议使用多个无关词语组成长口令。</span>
      </div>
...
.security-mark {
  ...
  background: color-mix(in srgb, var(--color-warning) 12%, var(--mrr-card));
  border: 1px solid color-mix(in srgb, var(--color-warning) 22%, var(--mrr-border));
  border-radius: var(--mrr-radius-xl);
  place-items: center;
}

.card-heading {
  margin: var(--mrr-space-5) 0;
}
