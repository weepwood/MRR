...
<template>
  <main class="password-page">
    <header class="page-toolbar">
      <div class="environment-badge"><span class="status-dot" />MRR Credential Setup</div>
      <ColorScheme />
    </header>

    <section class="password-card" aria-labelledby="password-change-title">
      <div class="security-mark"><FaIcon name="i-ri:shield-keyhole-line" /></div>
      <div class="card-heading">
        <span>Required security action</span>
        <h1 id="password-change-title">首次登录，请设置新密码</h1>
        <p>
      ...
      <div class="card-actions">
        ...
        <el-button type="primary" :loading="loading" @click="submit">保存新密码并重新登录</el-button>
      </div>
    </section>
  </main>
</template>
...
.security-mark { display: grid; width: 52px; height: 52px; font-size: 25px; color: var(--color-warning); background: color-mix(in srgb, var(--color-warning) 12%, var(--mrr-card)); border: 1px solid color-mix(in srgb, var(--color-warning) 22%, var(--mrr-border)); border-radius: var(--mrr-radius-xl); place-items: center; }
...
.visibility-button { display: grid; padding: 4px; color: var(--mrr-muted-foreground); cursor: pointer; background: transparent; border: 0; place-items: center; }
