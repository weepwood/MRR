  ...
  <div class="permissions-view">
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">集中查看角色、权限点和当前账号的可见范围，默认只读展示。</p>
      </div>
    ...
    <div class="permission-layout">
      <el-card class="panel permission-card" shadow="never">
        <template #header>
          <div class="panel-header">
            <div>
              <h3 class="panel-title">角色清单</h3>
              <p class="panel-subtitle">按角色查看对应权限集合与排序信息。</p>
            </div>
            <span class="badge">{{ filteredRoles.length }} 个角色</span>
          </div>
        </template>
        ...
        <template #header>
          <div class="panel-header">
            <div>
              ...
              <p class="panel-subtitle">按模块聚合展示当前系统已定义的权限点。</p>
            </div>
            <span class="badge">{{ groupedPermissions.length }} 个模块</span>
          </div>
        </template>
    ...
    <el-card class="panel section" shadow="never">
      <template #header>
        <div class="panel-header">
          <div>
            <h3 class="panel-title">当前账号权限</h3>
            <p class="panel-subtitle">结合登录态展示当前可访问范围。</p>
          </div>
        </div>
      </template>
...
const currentUser = computed(() => ({
  displayName: userStore.account,
  username: userStore.account,
  roleName: '',
  roleCode: '',
  status: userStore.isLogin ? 'active' : 'inactive',
  permissions: userStore.permissions
}))
const isAdmin = computed(() => {
  // 简单判断：如果有token且权限数组包含admin相关权限，则认为是管理员
  return userStore.isLogin && userStore.permissions.some(perm => perm.includes('admin'))
})
...
const getPermissionList = (item: any) => normalizePermissions(item)
