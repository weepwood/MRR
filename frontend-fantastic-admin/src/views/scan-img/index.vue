    ...
    <div class="split-layout">
      ...
      <div class="left-pane">
        ...
        <div class="left-top">
          ...
          <!-- 加载中提示 -->
          <div v-if="idSearchLoading" class="id-results-empty">
            <el-icon class="is-loading"><i class="i-ant-design:loading-outlined" /></el-icon>
            查询中...
          </div>
          <!-- 病案结果列表 -->
          <div class="id-results" v-else-if="idSearchResults.length">
            <div
          ...
          <div class="id-results-empty" v-else-if="!idSearchLoading && initDone">
            <p>未查询到相关住院记录</p>
            <p class="id-card-hint">身份证：{{ displayIdCard }}</p>
          </div>
        </div>
      ...
      <div class="right-pane">
        ...
        <div class="viewer-split" ref="viewerSplitRef">
          ...
          <div class="thumbs" ref="thumbsContainer">
            ...
            <div v-if="!filteredImages.length && !isLoading" class="thumbs-empty">
              {{ idSearchResults.length ? '暂无图片' : '请先选择左侧病案' }}
            </div>
          </div>
            ...
            <div class="viewer-source" ref="viewerContainer" @wheel.prevent="onViewerWheel">
              ...
              <div v-else class="main-placeholder">
                <div class="placeholder-icon">
                  <i class="i-ant-design:file-image-outlined" style="font-size: 64px; color: #cbd5e1;" />
                </div>
                <p class="placeholder-text">
                  {{ idSearchLoading ? '正在查询病案...' : (idSearchResults.length ? '请从左侧选择病案' : '未找到病案记录') }}
                </p>
              </div>
            </div>
            <!-- 图片信息显示和类型选择器 -->
...
// ==================== 身份证相关 ====================
// 从路由参数取身份证号
const idCardParam = computed(() => String(route.params.idCard || ''))
// 脱敏显示（保留前3位和后4位）
const displayIdCard = computed(() => {
  const v = idCardParam.value
...
const searchByIdCard = async (idCard: string) => {
  if (!idCard) return
  idSearchLoading.value = true
  idSearchResults.value = []
  images.value = []
  try {
    const res = await getBAHByIdCard(idCard)
    const data = res.data
    idSearchResults.value = Array.isArray(data) ? data : []
    // 优先自动选中在可访问区间内的第一条
    const firstInRange = idSearchResults.value.find(r => isBahClickable(r.bah))
    if (firstInRange) {
      selectRecord(firstInRange)
    }
    else if (idSearchResults.value.length > 0) {
      // 没有区间内的记录时，选第一条但不自动加载（可能无权限）
      selectedRecord.value = idSearchResults.value[0]
    }
  }
  catch {
    ElMessage.error('身份证查询失败，请检查网络')
    idSearchResults.value = []
  }
  finally {
...
onMounted(async () => {
  ...
  // 核心入口：以身份证号触发查询
  if (idCardParam.value) {
    await searchByIdCard(idCardParam.value)
  }
  else {
    initDone.value = true
  }
...
// 路由参数变化时重新查询（支持在同页内切换身份证）
watch(idCardParam, async (newId) => {
  if (newId) {
    await searchByIdCard(newId)
  }
})
</script>
