...
<template>
  <section class="setting-section">
    <el-alert
      ...
      :closable="false"
      show-icon
      title="开发者模式会绕过普通认证并放宽跨域策略，只能用于隔离的开发和联调环境。"
    />
    <div class="setting-row">
      <div>
        <strong>兼容旧版无登录接口调用</strong>
        <p>开发环境和打包后的产物均可启用。无有效 JWT 时使用虚拟 dev / ADMIN 会话，正式生产环境必须关闭。</p>
      </div>
      <el-switch v-model="settings.developerModeEnabled" inline-prompt active-text="启用" inactive-text="关闭" />
    </div>
    <div class="developer-grid">
      <article>
        <span><FaIcon name="i-ri:shield-user-line" /></span>
        <div><strong>认证兼容</strong><p>无有效 Token 的普通 API 使用虚拟管理员身份，并记录开发者模式访问日志。</p></div>
      </article>
      <article>
        <span><FaIcon name="i-ri:global-line" /></span>
        <div><strong>跨域调试</strong><p>启用期间允许任意浏览器 Origin 发起调试请求，关闭后恢复配置白名单。</p></div>
      </article>
    </div>
  </section>
</template>
