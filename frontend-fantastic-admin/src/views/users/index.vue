...
const summaryCards = computed(() => [
  ...
  {
    label: '已启用',
    value: users.value.filter(item => normalizeStatus(item.status) === 'active').length,
    note: '当前页可正常登录账号',
  ...
  {
    label: '待修改密码',
    value: users.value.filter(item => item.mustChangePassword).length,
    note: '首次登录或管理员重置后待处理',
  ...
  {
    label: '已禁用',
    value: users.value.filter(item => normalizeStatus(item.status) === 'disabled').length,
    note: '当前页已停止登录账号',
...
const createRules = {
  username: [
    ...
    { pattern: /^[\w.-]+$/, message: '只能包含字母、数字、点、下划线和短横线', trigger: 'blur' },
  ],
  roleCode: [{ required: true, message: '请选择角色', trigger: 'change' }],
}
  ...
  <MrrPageShell width="fluid">
    ...
    <MrrDataTablePanel title="账号列表" description="临时密码只在创建或重置成功时展示一次。" icon="i-ant-design:unordered-list-outlined" :count="total">
      ...
      <el-table v-else :data="users" row-key="id">
        <el-table-column prop="username" label="用户名" min-width="150">
          <template #default="{ row }">
            <div class="user-identity">
              <span class="user-avatar">{{ String(row.displayName || row.username || '?').slice(0, 1) }}</span><strong>{{ row.username }}</strong>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="凭据状态" min-width="170">
          <template #default="{ row }">
            <el-tag :type="credentialStatus(row).type" effect="light" round>
        ...
        <el-table-column label="操作" width="245" fixed="right" align="right">
          <template #default="{ row }">
            <div v-if="canManage" class="row-actions">
              ...
              <el-button link type="warning" :disabled="isSelf(row.id) || normalizeStatus(row.status) === 'disabled'" @click="openReset(row)">
                重置密码
              </el-button>
              <el-button link type="danger" :disabled="normalizeStatus(row.status) === 'disabled' || isSelf(row.id)" @click="handleDisable(row)">
                禁用
              </el-button>
            </div>
    ...
    <el-dialog v-model="createVisible" title="创建用户" width="520px" :close-on-click-modal="false">
      ...
      <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-position="top" class="dialog-form">
        ...
        <el-form-item label="角色" prop="roleCode">
          <el-select v-model="createForm.roleCode" class="full-width">
            <el-option v-for="role in roles" :key="role.code" :label="role.name || role.code" :value="role.code!" />
          </el-select>
        </el-form-item>
        <el-form-item label="临时密码有效期">
          <el-input-number v-model="createForm.temporaryPasswordValidHours" :min="1" :max="168" /><span class="unit">小时</span>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="createVisible = false">
          取消
        </el-button><el-button type="primary" :loading="createSaving" @click="handleCreate">
          创建用户
        </el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="editVisible" title="编辑用户" width="480px" :close-on-click-modal="false">
      <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-position="top" class="dialog-form">
        ...
        <el-form-item label="角色" prop="roleCode">
          <el-select v-model="editForm.roleCode" class="full-width">
            <el-option v-for="role in roles" :key="role.code" :label="role.name || role.code" :value="role.code!" />
          </el-select>
        </el-form-item>
      ...
      <template #footer>
        <el-button @click="editVisible = false">
          取消
        </el-button><el-button type="primary" :loading="editSaving" @click="handleSaveEdit">
          保存修改
        </el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="resetVisible" title="重置用户密码" width="500px" :close-on-click-modal="false">
      ...
      <el-form ref="resetFormRef" :model="resetForm" :rules="resetRules" label-position="top" class="dialog-form">
        <el-form-item label="当前管理员密码" prop="administratorPassword">
          <el-input v-model="resetForm.administratorPassword" type="password" autocomplete="current-password" show-password />
        </el-form-item>
        <el-form-item label="临时密码有效期">
          <el-input-number v-model="resetForm.temporaryPasswordValidHours" :min="1" :max="168" /><span class="unit">小时</span>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="resetVisible = false">
          取消
        </el-button><el-button type="danger" :loading="resetSaving" @click="handleResetPassword">
          确认重置
        </el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="credentialVisible" :title="credentialTitle" width="520px" :close-on-click-modal="false" :show-close="false" :close-on-press-escape="false">
      ...
      <div class="credential-actions">
        <el-button @click="copyCredentialSummary">
          复制账号信息
        </el-button><el-button type="primary" @click="copyText(credentialResult?.temporaryPassword || '', '临时密码已复制')">
          复制临时密码
        </el-button>
      </div>
      <el-checkbox v-model="credentialAcknowledged" class="acknowledge">
        我已通过安全方式保存或交付临时密码
      </el-checkbox>
      <template #footer>
        <el-button type="primary" :disabled="!credentialAcknowledged" @click="closeCredentialResult">
          完成
        </el-button>
      </template>
    </el-dialog>
...
.users-state { display: grid; place-items: center; min-height: 280px; padding: var(--mrr-space-5); }
.user-identity { display: flex; gap: 10px; align-items: center; min-width: 0; }
.user-avatar { display: grid; flex: 0 0 30px; place-items: center; width: 30px; height: 30px; font-size: 12px; font-weight: 700; color: var(--color-primary); background: color-mix(in srgb, var(--color-primary) 9%, var(--mrr-card)); border: 1px solid var(--mrr-border); border-radius: 50%; }
...
.no-perm,
.unit { margin-left: 8px; font-size: 12px; color: var(--mrr-muted-foreground); }
...
@media (width <= 760px) {
  ...
  .users-filter__select { flex: 1 1 100%; width: 100%; }
