        ...
        <div class="left-bottom">
          ...
          <div class="type-list">
            <div class="type-item" :class="{ active: selectedType === 'all' }" @click="onSelectType('all')">
              ...
              <div class="type-select-btn" @click.stop="selectAllByType('all')" 
                :class="{ 
                  selected: isAllTypeSelected,
                  partiallySelected: isTypePartiallySelected('all')
                }">
                <el-icon v-if="isAllTypeSelected" ><CircleCheckFilled /></el-icon>
            ...
            <div v-for="item in typeDisplayList" :key="item.type" class="type-item"
              ...
              <div class="type-select-btn" @click.stop="selectAllByType(item.type)" 
                :class="{ 
                  selected: isTypeSelected(item.type), 
                  partiallySelected: isTypePartiallySelected(item.type),
                  disabled: item.count === 0 
                }">
                <el-icon v-if="isTypeSelected(item.type)"><CircleCheckFilled /></el-icon>
                <el-icon v-else><CirclePlusFilled /></el-icon>
              </div>
            </div>
         ...
         <!-- 多选工具栏  v-if="showPrintButton"-->
        <div class="multi-select-toolbar">
          <div class="toolbar-left">
            <button class="selection-count" @click="toggleSelectedFilter" :class="{ active: showSelectedOnly }">
              已选择 {{ selectedImages.size }} 张图片
            </button>
            <button class="toolbar-btn" @click="clearSelection">清除选择</button>
            <button class="toolbar-btn" @click="selectAllVisible">全选当前</button>
            <button class="print-btn" @click="goToPrintPage">
      ...
      <div class="right-pane">
        <div class="viewer-split" ref="viewerSplitRef">
          <div class="thumbs" ref="thumbsContainer">
            ...
            <template v-if="thumbsViewMode === 'icons'">
              <div v-for="(img, idx) in filteredImages" :key="img.cx || img.id || idx" class="thumb-item"
                ...
                :ref="el => thumbRefs[idx] = el" :data-index="idx"
                ...
                <div class="thumb-image-container">
                  ...
                  <div class="thumb-select-btn" @click.stop="toggleImageSelection(idx)"
            ...
            <template v-else>
              <div v-for="(img, idx) in filteredImages" :key="img.cx || img.id || idx" class="thumb-row"
                :class="{ active: idx === selectedImageIndex, selected: isImageSelected(img) }" :ref="el => thumbRefs[idx] = el" :data-index="idx"
                @click="selectImage(idx, 'click')">
                <div class="thumb-row-select-btn" @click.stop="toggleImageSelection(idx)"
                  :class="{ selected: isImageSelected(img) }">
                  <el-icon v-if="isImageSelected(img)"><CircleCheckFilled /></el-icon>
                  <el-icon v-else><CirclePlusFilled /></el-icon>
                </div>
          ...
          <div class="main-view">
            ...
            <div v-if="currentImage && !isPreviewOpen" class="main-meta" @click.stop="toggleTypePicker">
...
const showPrintButton = computed(() => selectedImages.value.size > 0)
const showSelectedOnly = ref(false) // 是否只显示选中的图片
...
const toggleImageSelection = (idx) => {
  ...
  const imgId = img.id || img.cx || `${img.pages}_${img.btype}` // 使用图片ID作为唯一标识
...
const clearSelection = () => {
  ...
  showSelectedOnly.value = false // 清除选择时也关闭筛选
}
...
const toggleSelectedFilter = () => {
  ...
  // 如果开启筛选，确保当前选中的图片在可视范围内
  if (showSelectedOnly.value && filteredImages.value.length > 0) {
...
const selectAllVisible = () => {
  filteredImages.value.forEach((img) => {
    const imgId = img.id || img.cx || `${img.pages}_${img.btype}`
    selectedImages.value.add(imgId)
  })
}
...
const isTypeSelected = (type) => {
  if (type === 'all') return isAllTypeSelected.value
  
  const typeImages = images.value.filter(img => img.btype === type)
  if (typeImages.length === 0) return false
  
  return typeImages.every(img => isImageSelected(img))
}

// 判断某个类型是否有图片被选中（部分选中）
const isTypePartiallySelected = (type) => {
  if (type === 'all') {
    // 对于"全部"类型，如果有图片被选中但不是全部选中，则为部分选中
    return selectedImages.value.size > 0 && !isAllTypeSelected.value
  }
  
  const typeImages = images.value.filter(img => img.btype === type)
  if (typeImages.length === 0) return false
  
  const selectedCount = typeImages.filter(img => isImageSelected(img)).length
  return selectedCount > 0 && selectedCount < typeImages.length
}

// 判断所有图片是否都被选中
const isAllTypeSelected = computed(() => {
  if (images.value.length === 0) return false
  return images.value.every(img => isImageSelected(img))
})

// 根据类型选中所有图片
const selectAllByType = (type) => {
  if (type === 'all') {
    // 如果全部类型已选中，则取消选中所有；否则选中所有
    if (isAllTypeSelected.value) {
      clearSelection()
    } else {
    ...
    // 获取该类型的所有图片
    const typeImages = images.value.filter(img => img.btype === type)
    if (typeImages.length === 0) return
    
    // 检查该类型是否已全部选中
    const isAllSelected = typeImages.every(img => isImageSelected(img))
    
    if (isAllSelected) {
      // 如果已全部选中，则取消选中该类型的所有图片
...
const goToPrintPage = () => {
  ...
  for (const imgId of selectedImages.value) {
    ...
    const img = images.value.find(img => {
      const currentImgId = img.id || img.cx || `${img.pages}_${img.btype}`
  ...
  // 将选中的图片数据存储到 sessionStorage
  sessionStorage.setItem('selectedImagesForPrint', JSON.stringify(selectedImageData))
  sessionStorage.setItem('printBah', searchBah.value)
  sessionStorage.setItem('printRecord', JSON.stringify(selectedRecord.value))
...
const filteredImages = computed(() => {
  let result = selectedType.value === 'all' ? images.value : images.value.filter(img => img.btype === selectedType.value)
...
const selectRecord = (rec) => {
  ...
  if (selectedImages.value.size > 0) {
    ElMessageBox.confirm('切换病案时，会清空当前选中图片，是否继续？', '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' }).then(() => {
      clearSelection()
      selectedRecord.value = rec; searchBah.value = rec.bah; images.value = []; selectedImageIndex.value = 0; loadImages()
    })
  } else {
    selectedRecord.value = rec; searchBah.value = rec.bah; images.value = []; selectedImageIndex.value = 0; loadImages()
  }
...
.type-item:hover .type-select-btn {
  opacity: 1;
  transform: scale(1);
}
...
.type-select-btn.partiallySelected {
  opacity: 1;
  transform: scale(1);
  background: #c7dcfe;
  border-color: #0b97f5;
...
.thumb-item:hover .thumb-select-btn {
  opacity: 1;
  transform: scale(1);
}

.thumb-select-btn.selected {
  opacity: 1;
  transform: scale(1);
  background: var(--success-color);
  border-color: var(--success-color);
...
.thumb-row:hover .thumb-row-select-btn {
  opacity: 1;
  transform: scale(1);
}

.thumb-row-select-btn.selected {
  opacity: 1;
  transform: scale(1);
  background: var(--success-color);
  border-color: var(--success-color);
...
.thumb-row-select-btn:hover {
  transform: scale(1.1);
  box-shadow: var(--shadow-sm);
}

.thumb-row-select-btn .el-icon {
  font-size: 10px;
}
