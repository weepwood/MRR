...
// API 工具函数导入
import { getImgApiByBah, downloadBah, getBAHByIdCard, updateImgType } from '@/utils/api'
 ...
 * @returns {string} 类型名称
 */
const TYPE_NAME_MAP = Object.freeze({ 1: '病案首页', 2: '病程记录', 3: '手术记录', 4: '术后病程录', 5: '护理记录', 6: '会诊单', 7: '特殊检查', 8: '检验单', 9: '医嘱', 10: '体温单', 12: '出院记录', 13: '大病历', 14: '其它' })
...
const getTypeName = (type) => TYPE_NAME_MAP[type] || `类型${type}`
 ...
 */
const formatDate = (dateString) => { const date = new Date(dateString); return Number.isNaN(date.getTime()) ? '-' : dateFormatter.format(date) }
...
onMounted(async () => {
  ...
  try {
    if (window.ResizeObserver && thumbsContainer.value) {
      thumbsResizeObserver = new ResizeObserver(() => {
        try { thumbsPaneWidth.value = thumbsContainer.value.clientWidth || thumbsPaneWidth.value } catch (e) {}
      })
      thumbsResizeObserver.observe(thumbsContainer.value)
    }
  } catch (e) {}
  if (urlIdCard) {
...
onUnmounted(() => {
  ...
  try { if (thumbIO) { thumbIO.disconnect(); thumbIO = null } } catch (e) {}
  try { if (thumbsResizeObserver) { thumbsResizeObserver.disconnect(); thumbsResizeObserver = null } } catch (e) {}
})
...
.main-meta {
  ...
  opacity: 0.8;
  /* 移除 pointer-events: none，允许点击类型选择器 */
}
...
/* ==================== 低版本浏览器兼容处理 ==================== */
/* 针对不支持 gap 属性的旧浏览器，使用 margin 模拟间距（仅在不支持 gap 时生效） */
