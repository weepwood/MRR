...
def regex_replace(path: str, pattern: str, new: str, flags: int = 0) -> None:
    content = read(path)
    updated, count = re.subn(pattern, lambda _: new, content, count=1, flags=flags)
    if count != 1:
        raise SystemExit(f'{path}: pattern count={count}: {pattern}')
    write(path, updated)
...
def patch_installer() -> None:
    regex_replace(
        ...
        r"\s*\[Parameter\(Mandatory = \$true\)\]\s*\[string\]\$WinSWPath,\s*\[Parameter\(Mandatory = \$true\)\]\s*\[string\]\$NginxPath,",
        "\n    [string]$WinSWPath,\n\n    [string]$NginxPath,",
    )
    replace(
        'deploy/windows/install.ps1',
...
def patch_workflow() -> None:
    ...
    runtime_step = r'''      - name: Download pinned Windows runtime
        shell: bash
        run: |
          set -euo pipefail
...
'''
    replace(
        ...
        runtime_step + '      - name: Test and package backend\n',
    )
    replace(
        '.github/workflows/windows-release-package.yml',
...
def patch_docs() -> None:
    replace(
        ...
        '生产服务器只运行 PostgreSQL、JDK、Nginx、WinSW 和 MRR 发布包',
        '生产服务器只需要预装 PostgreSQL 和 JDK；Nginx 与 WinSW 已固定版本并包含在 MRR Windows 离线包中',
    )
    replace(
        'deploy/windows/README.md',
        '- Windows 版 Nginx；\n- WinSW。',
    ...
    replace(
        'vitepress-doc/internal/windows-deployment.md',
        '<!-- old -->维护模式由 Nginx 返回 503 页面，后端仍可完成已进入的请求，本机 Actuator 保持可用。',
        '',
    )
    replace(
        'vitepress-doc/internal/windows-deployment.md',
