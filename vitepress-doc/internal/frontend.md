...
Node.js 必须满足 `^24.15.0 || >=26.0.0`，仓库推荐 Node.js 22。
...
Chrome 86 的兼容目标是保证系统可访问，并支持登录、菜单导航、查询、分页、详情、编辑、设置和影像档案袋基础浏览等核心操作。Chrome 86 不支持 `color-mix()`、容器查询和动态视口单位等部分视觉增强能力，相关效果通过兼容样式降级，不要求与最新版 Chrome 完全一致。

兼容目标同时声明在：

- `package.json` 的 `browserslist`，用于 Autoprefixer 等 CSS 工具。
- `vite/plugins.ts` 的 `modernTargets`，用于固定 JavaScript 构建目标并按实际用法注入运行时 polyfill。
- `vite.config.ts` 的 `build.cssTarget`，用于固定 CSS 构建目标并通过 Lightning CSS 降级兼容语法。
...
必须使用真实 Chrome 86 或固定版本虚拟机验证生产构建，不能只修改 User-Agent 或只测试 Vite 开发服务器。`pnpm test:compat:chrome86` 只校验构建产物、polyfill 加载顺序和关键 CSS 降级，不替代真实浏览器回归。
