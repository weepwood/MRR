...
MRR 前端支持安装为渐进式 Web 应用（PWA），可从 Edge、Chrome 等浏览器安装到 Windows 桌面，并以独立窗口运行。PWA 是增强能力，不是 MRR 主系统的运行前提。
...
- MRR 通过普通内网 HTTP 访问时，登录、查询、影像档案袋等主业务仍可正常运行；
- 在服务器本机访问 `http://localhost:8002` 可以验证完整 PWA；
- 从其他电脑访问 `http://服务器IP:8002` 时，浏览器会自动降级为普通 Web 应用，不启用 Service Worker、离线页和标准 PWA 安装能力；
- Edge 仍可通过“将此站点安装为应用”提供独立窗口和桌面快捷方式，但这不等同于完整 PWA；
- 受统一管理的终端可以通过 Edge 企业策略为单一 HTTP 来源提供例外，但 HTTP 流量仍未加密，只适合作为受控内网过渡方案；
- 内网正式启用完整 PWA 时，应配置受终端信任的 HTTPS 证书。
...
1. 使用 Edge 或 Chrome 打开 MRR 的受信任 HTTPS 地址；
2. 完成登录并确认系统可以正常访问；
...
在 Edge 中打开 MRR，选择“更多工具 → 应用 → 将此站点安装为应用”。该方式可获得独立窗口和快捷方式，但不提供 Service Worker 离线能力。
...
在普通 HTTP 客户端上，Service Worker 未注册属于预期降级，不影响 MRR 主业务。
